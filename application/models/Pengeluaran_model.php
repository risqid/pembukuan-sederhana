...
class Pengeluaran_model extends CI_Model
{
    function show()
    {
        $data = $this->db->order_by('id', 'desc')->get_where('kas', ['arah' => 0])->result_array();
        return $data;
    }
    ...
    function edit($saldo, $data)
    {
        ...
        if ($data['keluar'] > $oldData['keluar']) {
            $selisih = $data['keluar'] - $oldData['keluar'];
            $saldo -= $selisih;
            $data['saldo'] = $saldo;
        ...
        } elseif ($oldData['keluar'] > $data['keluar']) {
            $selisih = $oldData['keluar'] - $data['keluar'];
            $saldo += $selisih;
            $data['saldo'] = $saldo;
            $this->db->where('id', $data['id'])->update('kas', $data);
